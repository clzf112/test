...
from guns_ci import find_issue_key, find_issue_key_in_payload, first_non_empty, issue_url, parse_metadata_file, read_json, write_json, write_text
...
def detect_issue_key(explicit_issue_key: str, head_ref: str, ref_name: str, event_path: str) -> str:
    return first_non_empty(find_issue_key(explicit_issue_key), find_issue_key(head_ref), find_issue_key(ref_name), find_issue_key_in_payload(event_path))
...
def build_comment(execution_issue_key: str, source_issue_key: str, selected_test: str, github_run_url: str, github_sha: str, run_category: str) -> str:
    return '\n'.join([
...
def build_execution_payload(execution_issue_key: str, source_issue_key: str, selected_test: str, github_run_url: str, github_sha: str, run_category: str) -> dict[str, object]:
    return {
        ...
        'tests': [
            {
                ...
                'comment': build_comment(execution_issue_key, source_issue_key, selected_test, github_run_url, github_sha, run_category),
            }
        ],
    }
...
def main() -> int:
    ...
    github_repository = env_default('GITHUB_REPOSITORY', 'clzf112/test')
    ...
    selected_test = env_default('TEST_CLASS_SELECTOR', metadata_value(metadata_path, 'Selected test') or 'unknown-test-selector')
    ...
    try:
        ...
        if not source_issue_key:
            result['message'] = 'Skipping Xray import because there is no Jira Test issue key.'
            write_text(summary_path, summary_text(result))
            write_json(result_path, result)
            return 0
        if not explicit_execution_key:
            result['message'] = 'Skipping Xray import because there is no stable Xray Test Execution key.'
            write_text(summary_path, summary_text(result))
            write_json(result_path, result)
            return 0
        xray_status = xray_status_for_run(run_category)
        if not xray_status:
            result['message'] = 'Skipping Xray import because the run did not produce a business test result that should be recorded.'
            write_text(summary_path, summary_text(result))
            write_json(result_path, result)
            return 0

        payload = build_execution_payload(explicit_execution_key, source_issue_key, selected_test, github_run_url, github_sha, run_category)
        auth_payload = json.dumps({'client_id': xray_client_id, 'client_secret': xray_client_secret}).encode('utf-8')
        token_bytes = http_request(f'{xray_base_url}/api/v2/authenticate', 'POST', {'Content-Type': 'application/json'}, auth_payload)
        auth_token = token_bytes.decode('utf-8').strip().strip('"')
        response_bytes = http_request(
            f'{xray_base_url}/api/v2/import/execution',
            'POST',
